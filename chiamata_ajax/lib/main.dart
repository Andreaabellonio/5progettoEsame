...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _letturaDati(String codice) async {
    print("LETTURA DATI");
    print(codice);
    var url = "http://192.168.1.53:13377/prova";
    var params = {"codice": codice.toString()};
    print(params);
    http.post(Uri.encodeFull(url),
        body: json.encode(params),
        headers: {"Accept": "application/json",  HttpHeaders.contentTypeHeader: "application/json"}).then((response) {
      Map data = jsonDecode(response.body);
      print(data);
      setState(() {
        nome = data["nome"];
        qta = data["qta"];
        allergie = data["tracce"];
        image = data["urlImage"];
      });
    });
  }
