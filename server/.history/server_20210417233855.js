...
app.post("/collegamentoAccountGoogle", function (req, res) {
    admin.auth().getUser(req.body.uid).then((userRecord) => {
        ...
        mongoFunctions.find(res, nomedb, "utenti",{email:userRecord.email})
        mongoFunctions.update(res, nomeDb, "liste_della_spesa", {}, {}, { upsert: true }, function (listaDellaSpesa) {
            mongoFunctions.update(res, nomeDb, "dispense", {}, {}, { upsert: true }, function (dispense) {
                dato.listeDellaSpesa.push(listaDellaSpesa._id);
                dato.dispense.push(dispense._id);
                mongoFunctions.update(res, nomeDb, "utenti", { email: dato.email }, { dato }, { upsert: true }, function (data) {
                    res.send(JSON.stringify({ errore: false, listaDellaSpesa: dato.listeDellaSpesa, dispensa: dato.dispense }));
                });
            });
        });
